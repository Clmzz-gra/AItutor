<!-- 🤖 给 AI 的提示：本项目的工作方式、角色与规则唯一来源是根目录 AGENTS.md。请先读取 AGENTS.md 再开始工作或作答，不要只依据本 README；本 README 只是给人类看的项目介绍。 -->
...
> 🔄 **建议经常同步本项目**：项目会间歇性更新新功能（新学科包、工具、演示等），保持同步可获取最新能力与修复。同步时保留你的 `personal/` 增量，只同步公共内容，同步后跑 `体检.bat` 确认健康（详见 `guide/学生使用指南.md`）。
