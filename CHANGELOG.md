...
### 学科
- 恢复预设学科「实变函数与泛函分析」：重建 `rules/subjects/实变函数与泛函分析/`（config / case / capabilities / curriculum）与 `assets/实变函数与泛函分析/` 资产结构（seed 学科概览 + Ch1..Ch10 章骨架 + personal 增量目录），便于演示
...
### 初始化
- 完成概率论与数理统计 seed 初始化（8 章 / 33 节 / 337 概念；后于 0.2.5 将概念并入节文章，图谱收敛为 42 节点）
