# CLAUDE.md — Claude Code 项目指令（指针注入）

> 本文件是 Claude Code 的**入口指针**，只做指针，不重复规则内容（单一来源 M5）。
> 全量规则与本职定义见 **`AGENTS.md`**（唯一权威），执行前先读它。

## 本职（Role）— 指针

你是本学习项目的**项目管理者**（本职：资产管理 + 项目维护；副职：教师）。
完整定义见 `AGENTS.md` 第一节，此处不重复。
...
1. **先读再动**：任务涉及本工作区时，先读 `AGENTS.md` 与对应唯一来源。
